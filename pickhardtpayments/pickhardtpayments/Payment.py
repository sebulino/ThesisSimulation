...
class Payment:
    ...
    def _prepare_integer_indices_for_nodes(self):
        ...
        for k, node_id in enumerate(self._uncertainty_network.network.nodes()):
            if k == "036d4c098b3554157ecc8b162daafe250eb8d89e6f18ce471b264cca536e3596b0":
                print("!!!!!!!!!")
        ...
        for attempt in self.filter_attempts(AttemptStatus.PLANNED):
            ...
            for uncertainty_channel in iter(attempt.path):
                ...
                if not return_channel:
                    logger.warning("back channel missing for ", uncertainty_channel.short_channel_id)
            ...
            if not success_of_probe:
                logger.debug("failed sending {:,} through channel {}-{}".format(attempt.amount, erring_channel.src[0:4], erring_channel.dest[0:4]))
                err_ch = self.oracle_network.get_channel(erring_channel.src, erring_channel.dest, erring_channel.short_channel_id)
                logger.debug("which has {:,.0f} sats liquidity with capacity {:,}".format(err_ch.actual_liquidity, err_ch.capacity))
    ...
    def convert_node_path_to_attempt_path(self, graph: nx.MultiDiGraph, nodes: list) -> list:
        path = []
        i = 0
        while i < len(nodes)-1:
            path.append(self.uncertainty_network.get_channel(nodes[i], nodes[i+1], graph[nodes[i]][nodes[i+1]][0]['short_id']))
            i += 1
        return path
    ...
    def get_dijkstra_path(self, graph, weight: str):
        payment_rounds = 0
        success = False
        # TODO: if no elimination of edges, no loop necessary.
        #  Will always be the same path. Until then, break at 1
        while payment_rounds < 1 and not success:
            payment_rounds += 1
            try:
